import pandas as pd
from rdflib import Literal, RDF
from rdflib.namespace import XSD,SKOS
from rdflib.term import URIRef
from config import Config
import urllib.parse

class Institution:

    def __init__(self,g,file=None):
        self.df = pd.read_csv(file,sep="\t",quotechar='"')
        self.g = g

    def __init_graph(self):
        self.g.bind("cv-ai4dm",Config.cv_ai4dm)
        self.g.bind("skos",Config.skos)
        self.g.bind("xsd",Config.xsd)
        self.g.bind("owl",Config.owl)
        self.g.bind("rdf", Config.rdf)
        self.g.bind("rdfs",Config.rdfs)
        self.g.bind("ai4dm", Config.ai4dm)
        self.g.bind("cv", Config.cv)
        self.g.bind("ai4dm-data", Config.ai4dm_data)


    def create_triples(self,g):
        for i, row in self.df.iterrows():
            in_name = row['name']
            region_code = row['region']
            type = row['type']
            id = row['id']

            instance_name = urllib.parse.quote(f"institution-{in_name}")

            g.add((URIRef(Config.ai4dm_data + instance_name), RDF.type,URIRef(Config.ai4dm + "Institution")))
            g.add((URIRef(Config.ai4dm_data + instance_name), SKOS.prefLabel, Literal(in_name,datatype=XSD.string)))
            g.add((URIRef(Config.ai4dm_data + instance_name), SKOS.hiddenLabel,Literal(id, datatype=XSD.string)))

            if region_code is not None and isinstance(region_code,str):
                g.add((URIRef(Config.ai4dm_data + instance_name), URIRef(Config.ai4dm + "hasRegion"),
                       URIRef(Config.ai4dm_data + "region-"+region_code)))

            if type is not None and isinstance(type,str):
                g.add((URIRef(Config.ai4dm_data + instance_name), URIRef(Config.ai4dm + "hasInstitutionType"),
                         URIRef(Config.ai4dm_data + "institution-"+type)))





        return g




